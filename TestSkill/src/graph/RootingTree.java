...
public class RootingTree {
  ...
  private static TreeNode buildTree(List<List<Integer>> graph, TreeNode root) {
    for(int childId : graph.get(root.id())) {
      if(root.parent != null && childId == root.parent.id()) {
        continue;
      }
      TreeNode childTree = new TreeNode(childId, root);
      root.addChildren(childTree);
      buildTree(graph, childTree);
    }
    return root;
  }
